...
Duplicate Check: Verifies if CLIENT_NAME conflicts with existing data.

SSH Check: Checks/Generates SSH keys and copies them to the server (ssh-copy-id).

Deployment: Installs the snapshot-agent.sh script on the server.

Auto-Hardening: Locks the SSH key in authorized_keys to this specific CLIENT_NAME.
...
Check Logs: Look for I/O errors in dmesg or /var/log/syslog.

Retry: Run the command again to rule out transient network issues.
...
Boot into the system (Wait for timeout).
...
Recreate missing empty directories (/proc, /sys) if they were not auto-created by the script's EXCLUDE_MOUNTPOINTS feature.
...
Prepare: Agent creates <backup_root>/daily.0.tmp.

Transfer: Client rsyncs data directly into daily.0.tmp.

Commit: Agent rotates old snapshots and renames .tmp to daily.0.
...
Security Wrapper (Logical Defense): Use the wrapper to lock SSH keys to specific client names. This prevents accidental overwrites or "spoofing" of other clients.
